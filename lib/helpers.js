...
export async function fetchViaFlaresolverr(url, session = 'default') {
  ...
  try {
    ...
    console.log(`FlareSolverr returned ${body.length} bytes, status ${status} for ${url}`);
    ...
    if (isCloudflare) {
      ...
      if (cookie.cf_clearance === 'none' && cookie.cookies.length === 0) {
        ...
        if (response.ok) {
          ...
          // Direct fetch failed despite no CF - try FlareSolverr direct
          console.log('Direct fetch still failed, trying FlareSolverr direct fetch');
      ...
      if (stillBlocked || response.status === 403) {
        ...
        if (!retryResponse.ok) {
          // Last resort: fetch the actual URL via FlareSolverr directly
          console.log('Fresh cookie also failed, trying FlareSolverr direct fetch');
          const flareResponse = await fetchViaFlaresolverr(url, 'skidrowreloaded');
          if (flareResponse && flareResponse.ok) {
            resetSkidrowCircuit();
            return flareResponse;
