  ...
  try {
    ...
    const response = await retryableFetch(flaresolverrUrl, {
      ...
      body: JSON.stringify({
        ...
        userAgent: 'GameSearch-API-v2/2.0',
        // Provide common headers to emulate a browser request
        headers: {
          'Accept': 'application/json, text/plain, */*',
...
function buildOnlineFixPost({ id, title, link, date, image, description, excerpt }) {
  const parsed = parseOnlineFixLink(link);
  return {
    id: `onlinefix_${id || parsed.id || parsed.slug || Date.now()}`,
    originalId: id || parsed.id || '',
    title: decodeBasicHtmlEntities(title || 'No title'),
